# -*- coding: utf-8 -*-
"""
Copyright 2018, 2019 Surface Concept GmbH
...
provides all low-level functions supported by this wrapper. Additionally, this 
module defines structures from the scTDC_types.h header file. The functions 
defined in the scTDC class aim to match the original scTDC C API as close as 
possible, except for a few cases where they have been changed for more 
convenient use (sc_get_err_msg, for example).

A higher-level API is defined in the Device and Pipe classes, which facilitates
usage of 1D,2D,3D histogram pipes, statistics data, and time histograms of 
stand-alone TDCs.

Concerning the low-level access : 
...
DLD_IMAGE_XY_EXT. These are to be used for the pipe_type parameter in
sc_pipe_open2.
...
Additions 2020-05-14 : added wrapper for a separate library to save DLD events 
                       to HDF5
"""
